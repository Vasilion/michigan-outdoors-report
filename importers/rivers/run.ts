...
import { fetchFlowlines, fetchTroutRegs } from "./fetch";
import { knownRiverNames, normalizeRivers, troutDesignationsByName } from "./normalize";
import type { NormalizedRiver, RiverSeed, TroutDesignation } from "./normalize";
import {
  deleteStaleRivers,
  linkRiverRecords,
  readRiverSeeds,
    ...
    return fetchFlowlines().then(
      (flowlines: readonly GeoJsonFeature[]): Promise<RunResult> =>
        fetchTroutRegs().then((regs: readonly GeoJsonFeature[]): Promise<RunResult> => {
          ...
          const rivers: readonly NormalizedRiver[] = normalizeRivers(seeds, known, trout);
          ...
          process.stdout.write(
            `[rivers] ${rivers.length} river pages seeded, ${designated} designated trout streams\n`,
          );
          const runStartedAt: string = new Date().toISOString();
          return upsertRivers(client, rivers)
            .then((upserted: number): Promise<number> =>
              deleteStaleRivers(client, runStartedAt).then((removed: number): number => {
                if (removed > 0) {
                  process.stdout.write(`[rivers] removed ${removed} stale river(s)\n`);
                }
                return upserted;
              }),
            )
